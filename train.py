...
def train(args):
    ...
    for userno in range(6, 13, 3):
        ...
        RBnumber = len(channel_interference)
        ...
        for average in range(1, args.averagenumber + 1):
            ...
            W = np.zeros((args.user_number, RBnumber)) 
        ...
        if len(np.where(finalq < 1)[0]) > 0:
            for iteration in range(args.iteration):
                ...
                if len(users_per_iteration[i]) > 0:
                    global_weights = global_model.state_dict()
                    for j, _user in enumerate(users_per_iteration[i]):
                        if iteration == 0 and j == 0:                
                            sum_weights = {}
                            for layer_name in global_weights.keys():
                                sum_weights[layer_name] = models[f"model_{_user}"].state_dict()[layer_name] * datanumber[_user]
                        else:
                            for layer_name in global_weights.keys():
                                sum_weights[layer_name] += models[f"model_{_user}"].state_dict()[layer_name] * datanumber[_user]
                    ...
                    for layer_name in global_model.state_dict():
                        sum_weights[layer_name] /= sum(datanumber((users_per_iteration[i])))                   
                                
                    global_model = global_model.load_state_dict(sum_weights)
